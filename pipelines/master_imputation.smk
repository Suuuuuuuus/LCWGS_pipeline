...
imputation_dir_new = config['imputation_dir'][-1]

rule new_all:
    input:
        vcf = expand("results/wip_vcfs/oneKG/vanilla/combined_sites/lc.chr{chr}.vcf.gz", chr = chromosome),
        impacc_v = expand("{imp_dir}impacc/all_samples/by_sample/all.v.impacc.tsv", imp_dir = imputation_dir_new)
