...
rule prepare_hla_vcf_merged_ref:
    ...
    shell: """
        ...
        sed -i 's/sample population group sex/SAMPLE POP GROUP SEX/g' {output.sample}
    """
...
###### Testing v3570 ref panel #####

rule prepare_hla_reference_panel_merged_ref_v3570:
    input:
        hla_types_panel = "results/hla_ref_panel/oneKG_mGenv1/oneKG_mGenv1_HLA_calls.tsv",
        ipd_igmt = f"{hla_ref_panel_indir}IPD-IMGT-HLA_v3570.zip",
        fasta = "data/references/concatenated/GRCh38_no_alt_Pf3D7_v3_phiX.fasta",
        genetic_map = f"{hla_ref_panel_indir}YRI/YRI-chr6-final.b38.txt.gz",
        hap = "results/hla_ref_panel/oneKG_mGenv1/hla_prepare_ref/oneKG_GAMCC.chr6.hap.gz",
        ...
        sample = "results/hla_ref_panel/oneKG_mGenv1/hla_prepare_ref/oneKG_GAMCC.chr6.samples"
    output:
        ref_panel = expand("results/hla/imputation/ref_panel/QUILT_prepared_reference_merged_ref_v3570/no_{id}/hla{hla_gene}haptypes.RData", hla_gene = hla_genes, allow_missing = True),
        exclude_sample_file = temp("results/hla/imputation/ref_panel/QUILT_prepared_reference_merged_ref_v3570/no_{id}/{id}.tsv")
    resources:
        mem = '50G'
    threads: 5
    conda: "sus"
    params:
        ...
        refseq = "/well/band/users/rbx225/software/QUILT/hla_ancillary_files/refseq.hg38.chr6.26000000.34000000.txt.gz",
        region_exclude_file = "/well/band/users/rbx225/software/QUILT/hla_ancillary_files/hlagenes.txt",
        hla_ref_panel_outdir = "results/hla/imputation/ref_panel/QUILT_prepared_reference_merged_ref_v3570/no_{id}/"
    shell: """
        echo {wildcards.id} >> {output.exclude_sample_file}
...
rule hla_imputation_merged_ref_v3570:
    input:
        bam = "data/bams/{id}.bam",
        ref_panel = "results/hla/imputation/ref_panel/QUILT_prepared_reference_merged_ref_v3570/no_{id}/hla{hla_gene}haptypes.RData"
    output:
        bamfile = temp("results/hla/imputation/QUILT_HLA_result_merged_ref_v3570/{id}/{id}.{hla_gene}.tsv"),
        imputed = "results/hla/imputation/QUILT_HLA_result_merged_ref_v3570/{id}/{hla_gene}/quilt.hla.output.combined.all.txt"
    resources:
        mem = '40G'
    threads: 4
    params:
        quilt_hla = tools['quilt_hla'],
        fa_dict = "data/references/concatenated/GRCh38_no_alt_Pf3D7_v3_phiX.dict",
        ref_dir = "results/hla/imputation/ref_panel/QUILT_prepared_reference_merged_ref_v3570/no_{id}/"
    conda: "sus"
    shell: """
        mkdir -p results/hla/imputation/QUILT_HLA_result_merged_ref_v3570/{wildcards.id}/{wildcards.hla_gene}/

        echo {input.bam} > {output.bamfile}

        {params.quilt_hla} \
        --outputdir="results/hla/imputation/QUILT_HLA_result_merged_ref_v3570/{wildcards.id}/{wildcards.hla_gene}/" \
        --bamlist={output.bamfile} \
        --region={wildcards.hla_gene} \
        --prepared_hla_reference_dir={params.ref_dir} \
        --quilt_hla_haplotype_panelfile={params.ref_dir}/quilt.hrc.hla.{wildcards.hla_gene}.haplotypes.RData \
...
rule prepare_hla_db:
    input:
        ipd_gen_files = '/well/band/users/rbx225/recyclable_files/hla_reference_files/alignments_v{IPD_IMGT_version}/{hla_gene}_gen.txt'
    output:
        output_db = '/well/band/users/rbx225/recyclable_files/hla_reference_files/v{IPD_IMGT_version}_aligners/{hla_gene}.ssv'
    resources:
        mem = '80G'
    threads: 4
    params:
...
###### Testing imputation method v3570 ######

rule prepare_hla_reference_panel_method_v3570:
    input:
        ...
        sample = f"{hla_ref_panel_indir}oneKG.samples"
    output:
        ref_panel = expand("results/hla/imputation/ref_panel/QUILT_prepared_reference_method_v3570/hla{hla_gene}haptypes.RData", hla_gene = hla_genes)
    resources:
        mem = '60G'
    threads: 4
    params:
...
rule hla_imputation_method_v3570:
    input:
        bam = "data/bams/{id}.bam",
    ...
    shell: """
        ...
        {params.quilt_sus_hla} \
        --outputdir="results/hla/imputation/QUILT_HLA_result_method_v3570/{wildcards.id}/{wildcards.hla_gene}/" \
        --bamlist={output.bamlist} \
        --region={wildcards.hla_gene} \
        --prepared_hla_reference_dir={params.ref_dir} \
        --quilt_hla_haplotype_panelfile={params.ref_dir}/quilt.hrc.hla.{wildcards.hla_gene}.haplotypes.RData \
